...
const VaRChart: React.FC<VaRChartProps> = ({ returns = [] }) => {
    ...
    useEffect(() => {
        ...
        const updateVaRValues = () => {
            ...
            const var95Value = (results[idx95] || 0) * 100;
            const var99Value = (results[idx99] || 0) * 100;
        ...
        for (let i = 0; i < bins; i++) {
            ...
            if (binStart < (results[idx95] || 0)) color = 'orange';
            if (binStart < (results[idx99] || 0)) color = 'red';

            histogramData.push({
                time: i as any, // Dummy time, cast to any to satisfy Time type
                value: count,
                color: color
            });
        }
