...
const CorrelationMatrix = () => {
    const [history, setHistory] = useState<Record<string, number[]>>({}); // { symbol: [price1, price2, ...] }
        ...
        const interval = setInterval(() => {
            ...
            for (let i = 0; i < assets.length; i++) {
                for (let j = 0; j < assets.length; j++) {
                    ...
                    const mean1 = s1.reduce((a: number, b: number) => a + b, 0) / len;
                    const mean2 = s2.reduce((a: number, b: number) => a + b, 0) / len;
                    ...
                    for (let k = 0; k < len; k++) {
                        const d1 = (s1[k] || 0) - mean1;
                        const d2 = (s2[k] || 0) - mean2;
                        num += d1 * d2;
                        den1 += d1 * d1;
                        den2 += d2 * d2;
                    }
