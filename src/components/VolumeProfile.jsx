 ...
 */
export const VolumeProfile = ({ candles = [] }) => {
    const profile = useMemo(() => {
        if (!candles || candles.length === 0) return { bins: [], poc: 0, valueAreaHigh: 0, valueAreaLow: 0 };
        ...
        candles.forEach(candle => {
            ...
            if (binIndex >= 0 && binIndex < numBins) {
                ...
                if (isBullish) {
                    bins[binIndex].buyVolume += candle.volume * 0.65;
                    bins[binIndex].sellVolume += candle.volume * 0.35;
                } else {
                    bins[binIndex].buyVolume += candle.volume * 0.35;
                    bins[binIndex].sellVolume += candle.volume * 0.65;
                }
            }
        });

        // Find Point of Control (highest volume)
        const poc = bins.reduce((max, bin) => bin.volume > max.volume ? bin : max, bins[0]);
        ...
        return {
            ...
            maxVolume: Math.max(...bins.map(b => b.volume))
        };
    }, [candles]);
