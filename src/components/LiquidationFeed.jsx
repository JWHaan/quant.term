...
import { Flame, TrendingDown, TrendingUp, Droplets } from 'lucide-react';
 ...
 */
const LiquidationFeed = ({ symbol = 'BTCUSDT' }) => {
    const [liquidations, setLiquidations] = useState([]);
    const [stats, setStats] = useState({ totalVol: 0, longVol: 0, shortVol: 0 });
    const wsRef = useRef(null);

    useEffect(() => {
        const handleLiquidation = (liq) => {
            ...
            <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
                {liquidations.length === 0 ? (
                    <div style={{
                        ...
                        <span style={{ fontSize: '9px' }}>Real-time !forceOrder@arr</span>
                    </div>
                ) : (
