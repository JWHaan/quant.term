...
const InstitutionalFeed = ({ symbol, onLargeTrade }) => {
    const { trades } = useBinanceWebSocket(symbol);

    // Filter for "Whale" trades (e.g., > 0.5 BTC approx $50k)
    const whaleTrades = trades.filter(t => (t.q * t.p) > 50000);
        ...
        return (
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '80px' }}>
                    ...
                    <span style={{ color: '#fff', fontWeight: '500' }}>{parseFloat(trade.p).toFixed(2)}</span>
                </div>

                <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', gap: '16px' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>{parseFloat(trade.q).toFixed(4)}</span>
                    <span style={{
                        color: isBuy ? 'var(--accent-primary)' : 'var(--accent-danger)',
                        width: '60px',
                        textAlign: 'right',
